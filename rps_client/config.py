...
class ConfigStore:
    ...
    def __post_init__(self) -> None:
        ...
        self.league = os.environ.get("RPS_LEAGUE", self.league)

    def _load(self) -> None:
        data = json.loads(self.path.read_text())
        self.token = data.get("token")
        self.api_url = data.get("api_url", DEFAULT_API_URL)
        self.league = data.get("league", DEFAULT_LEAGUE)

    def save(self) -> None:
        payload = {"token": self.token, "api_url": self.api_url, "league": self.league}
